...
\\
Three distribution types were modelled in our simulation based power analysis, with example draws respectively depicted in Figures \ref{fig:dist_norm}, \ref{fig:dist_gamma1}, and \ref{fig:dist_gamma2}:
\\
\\
1. In the case of bivariate normal data, under which our distributional assumptions are asymptotically met, this was specified with the two variable being standardised with mean $mu=zero$ and standard deviation $\sigma=1$:

$$\boldsymbol\mu = \begin{pmatrix} 0 \\ 0 \end{pmatrix}, \quad \boldsymbol\Sigma = \begin{pmatrix} 1 & \rho \\ \rho  & 1 \end{pmatrix}$$.  
 
To constrast with the 'assumptions met' normal scenario, two non-normal distribution scenarios were also considered.
\\
\\
...
\\
\\
3) An 'extreme skew' scenario based on a gamma distribution with mean 1 and dispersion 5.  This results in an extreme positive skew to the distribution, analogous to that of biological processes where most observations will be clustered around a certain value, however some outliers may be extremely elevated.
...
\begin{lstlisting}[float=h,caption={Example inspection of simulation results in database, while parallel processing is occurring.},label={lst:corrx_sql}]
